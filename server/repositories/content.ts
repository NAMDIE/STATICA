...
export interface ContentUserReference {
  id: string
  email: string
  displayName: string
...
interface ContentEntry {
  ...
  author: ContentUserReference | null
  createdBy: ContentUserReference | null
  updatedBy: ContentUserReference | null
  publishedBy: ContentUserReference | null
  createdAt: string
  updatedAt: string
...
function mapEntry(row: ContentEntryRow): ContentEntry {
  return {
    ...
    author: userReference(
      ...
      nullableString(row.author_email),
      nullableString(row.author_display_name),
      nullableString(row.author_role_slug),
      nullableString(row.author_role_name),
    ),
    createdBy: userReference(
      nullableString(row.created_by_user_id),
      nullableString(row.created_by_email),
      nullableString(row.created_by_display_name),
      nullableString(row.created_by_role_slug),
      nullableString(row.created_by_role_name),
    ),
    updatedBy: userReference(
      nullableString(row.updated_by_user_id),
      nullableString(row.updated_by_email),
      nullableString(row.updated_by_display_name),
      nullableString(row.updated_by_role_slug),
      nullableString(row.updated_by_role_name),
    ),
    publishedBy: userReference(
      nullableString(row.published_by_user_id),
      nullableString(row.published_by_email),
      nullableString(row.published_by_display_name),
      nullableString(row.published_by_role_slug),
      nullableString(row.published_by_role_name),
    ),
    createdAt: toIsoString(row.created_at),
    updatedAt: toIsoString(row.updated_at),
...
async function hydrateContentEntry(db: DbClient, row: ContentEntryRow): Promise<ContentEntry> {
  return (await getContentEntry(db, row.id)) ?? mapEntry(row)
}
...
export async function createContentEntry(
  ...
  `
  return hydrateContentEntry(db, rows[0])
}
...
export async function saveContentEntryDraft(
  ...
  const { rows } = await db<ContentEntryRow>`
    ...
    returning id, collection_id, title, slug, status, body_markdown, featured_media_id,
              ...
              published_at, deleted_at
  `
  return rows[0] ? hydrateContentEntry(db, rows[0]) : null
}
...
export async function updateContentEntryCollection(
  ...
  return { ok: true, entry: await hydrateContentEntry(db, rows[0]) }
}
...
export async function publishContentEntry(
  ...
  return db.transaction(async (tx) => {
    ...
    if (!updatedRow) throw new Error('content entry publish update failed')
    const hydratedEntry = await hydrateContentEntry(tx, updatedRow)
    return {
      entry: hydratedEntry,
      version: {
        id: versionId,
