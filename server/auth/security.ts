 ...
 *   - `clientIp`               — the forwarded client IP, when the proxy gives
 *                                us one.
 *
...
/**
 * Best-effort client IP. Reads `X-Forwarded-For` (set by Caddy) first.
 *
 * Note: when running without a proxy and Bun.serve doesn't surface the
 * client IP into the Request object, this returns null. Rate limiting can
 * coalesce that to a stable fallback key, but audit logs should not persist a
 * fake "unknown" IP address as if it were observed data.
 */
